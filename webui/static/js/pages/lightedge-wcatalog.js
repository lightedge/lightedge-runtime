...
function refresh_worker_catalog(){
  ...
  get_catalog_workers = function(data){
    ...
    REST_REQ(__LIGHTEDGE_WEBUI.ENTITY.WORKER.CATALOG).configure_GET({
  ...
  REST_REQ(__LIGHTEDGE_WEBUI.ENTITY.WORKER.WORKER).configure_GET({
    success: [ lightedge_log_response, get_catalog_workers ],
    error: [ lightedge_log_response, lightedge_alert_generate_error ]
  })
  .perform()
}
...
function alter_modal(key, descriptor){
  ...
  let f= function(){
    ...
    let modal= new WEBUI_Modal(__LIGHTEDGE_WEBUI.MODAL.TYPE.GENERIC,"worker_modal")
      .add_fields(fields)
...
function run_worker(name, modal){
  ...
  REST_REQ(__LIGHTEDGE_WEBUI.ENTITY.WORKER.WORKER).configure_POST({
    data: data,
