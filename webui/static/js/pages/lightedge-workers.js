...
WORKER_MODAL = __LIGHTEDGE_WEBUI.MODAL.ACTION
...
function show_active_workers(){
  ...
  REST_REQ(__LIGHTEDGE_WEBUI.ENTITY.WORKER.WORKER).configure_GET({
    success: [ lightedge_log_response, show_workers ],
...
function alter_modal(type, key, descriptor){
  let cf = __LIGHTEDGE_WEBUI.CORE_FUNCTIONS
  ...
  $.each(descriptor.manifest.params, function(key, val){
    ...
    if (cf._is_array(val.type)){
      type = __LIGHTEDGE_WEBUI.MODAL.FIELD.TYPE.SELECT
    } else {
      type = __LIGHTEDGE_WEBUI.MODAL.FIELD.TYPE.TEXT
    }
    let fstatic = false
    if (val.static){
        fstatic = true
  ...
  let modal= new WEBUI_Modal(__LIGHTEDGE_WEBUI.MODAL.TYPE.GENERIC,"worker_modal").add_fields(fields)
...
function edit_worker(uuid, modal){
  ...
  REST_REQ(__LIGHTEDGE_WEBUI.ENTITY.WORKER.WORKER).configure_PUT({
    key: uuid,
    data: data,
...
function stop_worker(uuid, modal){
  ...
  REST_REQ(__LIGHTEDGE_WEBUI.ENTITY.WORKER.WORKER).configure_DELETE({
    key: uuid,
    success: [ lightedge_log_response, lightedge_alert_generate_success,
      show_active_workers ],
    error: [ lightedge_log_response, lightedge_alert_generate_error ]
